...
public abstract class Bytes {
  ...
  public static short[] toShorttArray(byte[] input) {
    short[] ret = new short[input.length / 2];
    for (int i = 0; i < input.length / 2; ++i) {
      if ((input[i * 2 + 1] & 0x80) != 0) {
        ret[i] = (short) (-32768 + ((input[i * 2 + 1] & 0x7f) << 8) | (input[i * 2] & 0xff));
      } else {
        ret[i] = (short) (((input[i * 2 + 1] << 8) & 0xff00) | (input[i * 2] & 0xff));
      }
    }
    return ret;
  }
